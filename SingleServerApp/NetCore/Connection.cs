...
namespace NetCore
{
    public class Connection
    {
        ...
        public Action<SocketAsyncEventArgs> SendAction;
        public Action<Connection, string> CloseAction;
        ...
        {
            //解码消息存储对象
            byte[] buff = encoder.decode_packet(ref cache);
            //消息未接收全 退出数据处理 等待下次消息到达
            if (buff == null) { isReading = false; return; }

            //进行消息反序列化
            object message = encoder.decode_msg(buff);
            //TODO 通知应用层 有消息到达
            messageCenter.OnMessageReceive(this, message);
            //尾递归
            onData();
        }
        ////粘包拆包问题 ： 解决决策 ：消息头和消息尾。
